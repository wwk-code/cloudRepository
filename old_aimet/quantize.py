...
class ModelDataPipeline:
    ...
    def finetune(self,sim_model,train_sentences, train_labels, epochs=5, learning_rate=2e-5, batch_size=8):
        print('start qat')
...
# class TextEvaluator:
#     def __init__(self, dataset_name, subset, split, tokenizer, batch_size=8, num_workers=2):
...
#         return DataLoader(tokenized_dataset, batch_size=self.batch_size, num_workers=self.num_workers)
...
if __name__ == "__main__":
    ...
    accuracys = 0
    for i in range(iter):
        accuracy = pipeline.evaluate(sentences, labels, use_cuda=use_cuda,dummy_inputs=dummy_inputs)
        accuracys += accuracy
    end_time = time.time()
    print(f'normal: infer time: {(end_time - start_time)/3}  average_accuracy: {accuracys / 3}')
    ...
    # sim_model.export_onnx_model_and_encodings
    print('finished!')
